...
export default async function MentorPage() {
  ...
  const enrichedEvents = events.map((event) => {
    const totalParticipations = event.teams.flatMap(
      (team) => team.members,
    ).length;
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-8 shadow-sm">
          <h2 className="text-2xl font-semibold mb-8">Event Teams Overview</h2>
            ...
            {enrichedEvents.map((event) => (
              <MotionWrapper key={event.id}>
                <div className="border border-neutral-200 rounded-xl p-6 hover:shadow-md transition">
                  <h3 className="text-lg font-semibold mb-6">{event.title}</h3>
                  ...
                  {event.teams.length === 0 ? (
                    ...
                    <div className="space-y-5">
                      {event.teams.map((team) => {
                        ...
                        return (
                          <MotionWrapper key={team.id}>
                            <div className="border border-neutral-200 rounded-xl p-5 bg-neutral-50 hover:-translate-y-1 hover:shadow-md transition">
